...
const mini_promise = util.mini_promise
Page({
  ...
  onLoad: function(options) {
    ...
    /**
     * 使用教程：
     * mini_promise 用于调用微信提供的API，即以 wx. 开头的代码。
     * util.mini_promise 第一个参数是API名字，如果有参数，用花括号传入键值对，返回的是一个函数。
     * 第二个括号表示调用返回的函数，最终得到一个 Promise 对象。
     * 形如：util.mini_promise(wx.login)()
     * 因为 Promise 对象后面才能接 then 和catch。
     * 由于 then 和catch 要求传入的是函数，所以，在之后的串行机制中，不需要使用第二个括号。
     * 函数执行后，可以在 then 和 catch 中获取。
     */
    // util.mini_promise(wx.login)() // 手动添加括号调用 以返回 Promise 对象
    //   .then(util.mini_promise(wx.getLocation)) // 不需要返回值
    //   .then(util.mini_promise(wx.getStorage, {  // 传参
    //     key: 'logs'
    //   }))
    //   .then(res => {  // 需要返回值
    //     console.log('返回值', res)
    //     return util.mini_promise(wx.getLocation)() // 手动添加括号调用并返回给上层
    //   }) // 需要返回值 使用 return
    //   .catch(res => {
    //     console.log('异常处理', res)
    //   })
    /**
     * wx.request 和上面的用法相同，不同的是，mini_promise 中，会对请求状态值 statusCode 进行判断，如果不是 200，会抛出异常。
     */
    //所用的聚合API申请得治：https://www.juhe.cn
    // util.mini_promise(wx.request, {
    ...
    //   })
    //   .then(util.mini_promise(wx.request, {
    ...
    /**
     * 使用教程
     * 用法和 mini_promise 类似，不同的是参数传递方式，微信自带的API使用的是键值对参数，而自己定义的函数则不是规定的键值对，而是更为灵活的普通参数，考虑到动态传参的通用性，nano_promise 中，使用数组传参，nano_promise 追加了 resolve 和 reject方法后，会通过三点运算符将数组参数转换参数列表形式。
     */
    util.nano_promise(this.test1, ['kangour', 18])()
      .then(util.nano_promise(this.test2, ['kangour', 17]))
      .then(util.nano_promise(this.test1, ['jouns', 17]))
      .then(res => {
   ...
   * 但，一旦被 nano_promise 函数处理并使用，它将拥有串行执行的能力。
   *
   */
